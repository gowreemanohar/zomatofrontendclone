      ...
      <div className="lg:hidden bg-white shadow-lg  px-3 fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border">
        {allTypes.map((items) => (
          <Link to={`/${items.id}`} className="w-1/4">
            <div
              className={
                classnames("flex flex-col items-center text-xl ", {
                  "relative text-zomato-400": type === items.id,
                })
                // type === items.id
            ...
            >
              <div
                className={
                  classnames(
                    "flex flex-col justify-center items-center py-3 border-t-2 border-white",
                    { "w-full border-t-2 border-zomato-400": type === items.id }
                  )
                  // type === items.id
                  //   ? "flex flex-col justify-center items-center w-full border-t-2 border-zomato-400  py-3"
                  //   : " flex flex-col justify-center items-center py-3 border-t-2 border-white "
                }
              >
                {items.icon}
...
function LargeTabs() {
  ...
  return (
    <>
      <div className="hidden lg:flex gap-14 container px-32 my-8 mx-auto">
        {allTypes.map((items) => (
